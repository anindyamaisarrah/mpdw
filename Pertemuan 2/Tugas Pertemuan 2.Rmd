...
data1 <- import("C:\\SEM 5\\MPDW\\dataset pm2.5.xlsx", sheet = "INDY")
...
ggplot() + 
  ...
  labs(x = "Tanggal", y = "PM 2.5", color = "Legend") +
...
Selanjutnya dilakukan peramalan sebanyak 31 periode sesuai dengan jumlah data uji. Pada metode SMA, seluruh hasil peramalan untuk 31 periode ke depan akan memiliki nilai yang sama dengan hasil ramalan satu periode ke depan.

```{r}
data.gab<-cbind(
  aktual=c(data1.ts),
  pemulusan=c(data.sma,rep(NA,31)),
  ramalan=c(data.ramal,rep(data.ramal[length(data.ramal)],30)))
...
```{r}
ts.plot(data1.ts, xlab="Time Period ", ylab="PM 2.5", main= "SMA N=4 Data PM 2.5")
points(data1.ts)
...
Dalam hal ini nilai MAPE data latih pada metode pemulusan SMA sekitar 21%, nilai ini dapat dikategorikan sebagai nilai akurasi yang kurang baik. Selanjutnya dilakukan perhitungan nilai MAPE data uji pada metode pemulusan SMA.

#### Akurasi Data Uji

```{r}
#Menghitung nilai keakuratan data uji
forecast_sma <- tail(data.gab[,3], length(test_ma.ts))
error_test.sma <- test_ma.ts - forecast_sma
...
Perhitungan akurasi menggunakan data latih menghasilkan nilai MAPE yang lebih dari 10% sehingga nilai akurasi ini dapat dikategorikan sebagai kurang baik.
...
```{r}
ts.plot(data1.ts, xlab="Time Period ", ylab="PM 2.5", main= "DMA N=4 Data PM 2.5")
points(data1.ts)
...
#Menghitung nilai keakuratan data uji
forecast_sma <- tail(data.gab[,3], length(test_ma.ts))
error_test.dma <- test_ma.ts - forecast_sma
...
library(ggplot2)
ggplot() + 
  geom_line(data = training, aes(x = `tanggal`, y = `pm25`, col = "Data Latih")) +
  geom_line(data = testing, aes(x = `tanggal`, y = `pm25`, col = "Data Uji")) +
...
resid1<-training$`pm25`-ramalan1$fitted
head(resid1)
...
resid2<-training$`pm25`-ramalan2$fitted
head(resid2)
...
Berdasarkan nilai SSE, MSE, RMSE, dan MAPE di antara kedua parameter, nilai parameter $\lambda=0,2$ menghasilkan akurasi yang kurang baik dibanding $\lambda=0,7$ . Hal ini dilihat dari nilai masing-masing ukuran akurasi yang lebih besar. Berdasarkan nilai MAPE-nya, hasil ini dapat dikategorikan sebagai peramalan kurang baik.
...
```{r}
accuracy(ramalan1, testing$pm25)
accuracy(ramalan2, testing$pm25)
accuracy(ramalanopt, testing$pm25)
```
...
Hasil akurasi dari data latih didapatkan skenario 2 dengan lamda=0.6 dan gamma=0.3 memiliki hasil yang kurang baik. Selain itu untuk kedua skenario dapat dikategorikan peramalan yang kurang baik berdasarkan nilai MAPE-nya.

#### Akurasi Data Uji

```{r}
#Akurasi Data Testing
selisihdes1<-ramalandes1$mean-testing$`pm25`
selisihdes1

SSEtestingdes1<-sum(selisihdes1^2)
MSEtestingdes1<-SSEtestingdes1/length(testing$pm25)
MAPEtestingdes1<-sum(abs(selisihdes1/testing$pm25)*100)/length(testing$pm25)

selisihdes2<-ramalandes2$mean-testing$pm25
selisihdes2

SSEtestingdes2<-sum(selisihdes2^2)
MSEtestingdes2<-SSEtestingdes2/length(testing$pm25)
MAPEtestingdes2<-sum(abs(selisihdes2/testing$pm25)*100)/length(testing$pm25)

selisihdesopt<-ramalandesopt$mean-testing$pm25
selisihdesopt

SSEtestingdesopt<-sum(selisihdesopt^2)
MSEtestingdesopt<-SSEtestingdesopt/length(testing$pm25)
MAPEtestingdesopt<-sum(abs(selisihdesopt/testing$pm25)*100)/length(testing$pm25)
...
#Import data
library(rio)
data3 <- import("C:\\SEM 5\\MPDW\\dataset pm2.5.xlsx", sheet = "INDY")
data3.ts <- ts(data3$pm25)
```
...
training <- data3[1:124, 3]
testing  <- data3[125:155, 3]
...
#Akurasi Data Testing
# Nilai aktual
aktual <- as.numeric(testing.ts$testing.ts)

# Nilai ramalan, ambil 31 observasi pertama
ramalan1 <- as.numeric(forecast1$fit[1:length(aktual)])

# Hitung error
selisih1 <- aktual - ramalan1

# SSE
SSEtesting1 <- sum(selisih1^2)

# MSE
MSEtesting1 <- SSEtesting1 / length(aktual)

SSEtesting1
MSEtesting1

# Nilai ramalan model optimum
ramalan1.opt <- as.numeric(
  forecast1.opt[1:length(aktual)]
)

# Hitung error
selisih1.opt <- aktual - ramalan1.opt

# SSE
SSEtesting1.opt <- sum(selisih1.opt^2)

# MSE
MSEtesting1.opt <- SSEtesting1.opt / length(aktual)
...
#Akurasi Data Testing
# Nilai aktual
aktual2 <- as.numeric(testing.ts$testing.ts)

# Nilai ramalan, ambil 31 observasi pertama
ramalan2 <- as.numeric(forecast2$fit[1:length(aktual)])

# Hitung error
selisih2 <- aktual2 - ramalan2

# SSE
SSEtesting2 <- sum(selisih2^2)

# MSE
MSEtesting2 <- SSEtesting2 / length(aktual2)

SSEtesting2
MSEtesting2

# Nilai ramalan model optimum
ramalan2.opt <- as.numeric(
  forecast2.opt[1:length(aktual2)]
)

# Hitung error
selisih2.opt <- aktual2 - ramalan2.opt

# SSE
SSEtesting2.opt <- sum(selisih2.opt^2)

# MSE
MSEtesting2.opt <- SSEtesting2.opt / length(aktual2)
